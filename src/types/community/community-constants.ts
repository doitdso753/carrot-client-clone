import { HotIcon } from '@/assets/icons';
import type { CommunityComment, CommunityItem } from './community.ts';

export const COMMUNITY_CATEGORIES = [
  { code: 'popular', label: '인기글', icon: HotIcon },
  { code: 'all', label: '전체' },
  { code: 'restaurant', label: '맛집' },
  { code: 'event', label: '동네행사' },
  { code: 'pet', label: '반려동물' },
  { code: 'exercise', label: '운동' },
  { code: 'life', label: '생활/편의' },
  { code: 'lost', label: '분실/실종' },
  { code: 'hospital', label: '병원/약국' },
  { code: 'concern', label: '고민/사연' },
  { code: 'friend', label: '동네친구' },
  { code: 'moving', label: '이사/시공' },
  { code: 'realty', label: '주거/부동산' },
  { code: 'education', label: '교육' },
  { code: 'hobby', label: '취미' },
  { code: 'incident', label: '동네사건사고' },
  { code: 'scenery', label: '동네풍경' },
  { code: 'beauty', label: '미용' },
  { code: 'parenting', label: '임신/육아' },
  { code: 'general', label: '일반' },
] as const;

export const COMMUNITY_ITEMS: readonly CommunityItem[] = [
  {
    authorProfile: {
      location: '서울특별시 강남구 역삼동',
      nickname: '산책하는토끼',
      warmth: 38.2,
    },
    id: 1,
    title: '우리 동네에 새로 생긴 산책로를 소개해요',
    content: '날씨 좋은 날 가볍게 걷기 좋고 저녁 풍경도 정말 예뻐요.',
    category: '동네풍경',
    location: '역삼동',
    tags: '산책,야경,동네풍경',
    createdAt: '2026-08-21 14:00:00',
    likeCount: 6,
    commentCount: 5,
    viewCount: 0,
    imageUrl: 'https://picsum.photos/seed/community-walk/320/320',
  },
  {
    authorProfile: {
      location: '서울특별시 서초구 서초동',
      nickname: '서초마켓러버',
      warmth: 40.1,
    },
    id: 2,
    title: '주말에 열리는 동네 플리마켓 같이 가실 분',
    content: '이번 주 토요일 오전부터 공원 앞 광장에서 열린다고 합니다.',
    category: '동네행사',
    location: '서초동',
    tags: '플리마켓,주말행사,이웃모임',
    createdAt: '2026-08-21 13:00:00',
    likeCount: 6,
    commentCount: 5,
    viewCount: 0,
    imageUrl: 'https://picsum.photos/seed/community-market/320/320',
  },
  {
    authorProfile: {
      location: '인천광역시 연수구 송도동',
      nickname: '커피한잔',
      warmth: 37.4,
    },
    id: 3,
    title: '근처에서 발견한 분위기 좋은 카페',
    content: '조용해서 책 읽거나 작업하기 좋았어요. 디저트도 맛있습니다.',
    category: '맛집',
    location: '송도동',
    tags: '카페,디저트,작업공간',
    createdAt: '2026-08-21 12:00:00',
    likeCount: 6,
    commentCount: 5,
    viewCount: 0,
    imageUrl: 'https://picsum.photos/seed/community-cafe/320/320',
  },
  {
    authorProfile: {
      location: '서울특별시 관악구 신림동',
      nickname: '흰둥이를찾아요',
      warmth: 45.6,
    },
    id: 4,
    title: '잃어버린 강아지를 찾고 있습니다',
    content: '갈색 목줄을 하고 있는 흰색 소형견입니다. 제보 부탁드려요.',
    category: '분실/실종',
    location: '신림동',
    tags: '강아지,실종동물,제보',
    createdAt: '2026-08-21 10:00:00',
    likeCount: 6,
    commentCount: 5,
    viewCount: 0,
    imageUrl: 'https://picsum.photos/seed/community-dog/320/320',
  },
  {
    authorProfile: {
      location: '서울특별시 강남구 청담동',
      nickname: '저녁러너',
      warmth: 39.3,
    },
    id: 5,
    title: '퇴근 후 함께 러닝하실 동네 친구 구해요',
    content: '평일 저녁 8시쯤 천천히 5km 정도 달리는 모임입니다.',
    category: '운동',
    location: '청담동',
    tags: '러닝,운동모임,동네친구',
    createdAt: '2026-08-21 08:00:00',
    likeCount: 6,
    commentCount: 5,
    viewCount: 0,
    imageUrl: 'https://picsum.photos/seed/community-running/320/320',
  },
  {
    authorProfile: {
      location: '경기도 남양주시 다산동',
      nickname: '책읽는가족',
      warmth: 42.8,
    },
    id: 6,
    title: '아이와 함께 가기 좋은 동네 도서관',
    content: '유아 열람실이 넓고 주말 프로그램도 다양하게 운영하네요.',
    category: '임신/육아',
    location: '다산동',
    tags: '도서관,아이와함께,주말프로그램',
    createdAt: '2026-08-20 15:00:00',
    likeCount: 6,
    commentCount: 5,
    viewCount: 0,
    imageUrl: 'https://picsum.photos/seed/community-library/320/320',
  },
  {
    authorProfile: {
      location: '서울특별시 중구 신당동',
      nickname: '신당맛객',
      warmth: 41.2,
    },
    id: 7,
    title: '신당동 떡볶이 골목 새로 생긴 가게 다녀왔어요',
    content: '맵기 조절이 가능하고 튀김도 바삭해서 이웃분들께 추천합니다.',
    category: '맛집',
    location: '신당동',
    tags: '신당동,떡볶이,맛집추천',
    createdAt: '2026-08-28 19:20:00',
    likeCount: 18,
    commentCount: 7,
    viewCount: 132,
    imageUrl: 'https://picsum.photos/seed/sindang-community-food/320/320',
  },
  {
    authorProfile: {
      location: '서울특별시 중구 신당동',
      nickname: '남산산책러',
      warmth: 38.7,
    },
    id: 8,
    title: '저녁에 걷기 좋은 신당동 산책 코스',
    content: '성곽길 쪽으로 이어지는 길이 조용하고 야경도 예뻐요.',
    category: '동네풍경',
    location: '신당동',
    tags: '산책,야경,성곽길',
    createdAt: '2026-08-28 17:40:00',
    likeCount: 24,
    commentCount: 5,
    viewCount: 205,
    imageUrl: 'https://picsum.photos/seed/sindang-community-walk/320/320',
  },
  {
    authorProfile: {
      location: '서울특별시 중구 신당동',
      nickname: '튼튼이엄마',
      warmth: 45.3,
    },
    id: 9,
    title: '아이와 갈 만한 주말 체험 프로그램 있을까요?',
    content: '초등학생이 참여할 수 있는 미술이나 요리 프로그램을 찾고 있어요.',
    category: '임신/육아',
    location: '신당동',
    tags: '주말체험,아이와함께,정보공유',
    createdAt: '2026-08-28 14:10:00',
    likeCount: 9,
    commentCount: 12,
    viewCount: 98,
  },
  {
    authorProfile: {
      location: '서울특별시 중구 신당동',
      nickname: '신당러너',
      warmth: 39.9,
    },
    id: 10,
    title: '평일 아침 가볍게 같이 달리실 분',
    content: '충무아트센터 앞에서 만나 30분 정도 천천히 달리려고 합니다.',
    category: '운동',
    location: '신당동',
    tags: '아침러닝,운동,동네친구',
    createdAt: '2026-08-28 09:00:00',
    likeCount: 11,
    commentCount: 8,
    viewCount: 76,
    imageUrl: 'https://picsum.photos/seed/sindang-community-running/320/320',
  },
  {
    authorProfile: {
      location: '서울특별시 중구 신당동',
      nickname: '주인찾아요',
      warmth: 42.1,
    },
    id: 11,
    title: '신당역 근처에서 카드지갑을 주웠어요',
    content: '검은색 카드지갑입니다. 본인 확인 후 돌려드릴게요.',
    category: '분실/실종',
    location: '신당동',
    tags: '분실물,카드지갑,신당역',
    createdAt: '2026-08-27 21:30:00',
    likeCount: 15,
    commentCount: 4,
    viewCount: 143,
  },
];

export const COMMUNITY_COMMENTS: readonly CommunityComment[] = [
  {
    content: '누워있는 건가요?? 너무 안쓰럽네요.',
    createdAt: '2026-08-20 15:00:00',
    id: 1,
    likeCount: 2,
    location: '역무동',
    nickname: '피넛버터',
    profileImageUrl: 'https://picsum.photos/seed/community-comment-1/72/72',
    replyCount: 1,
    replies: [
      {
        content:
          '전 날까지는 움직여서 오늘 밥준건데 오늘은 기운이 없어 보여요.',
        createdAt: '2026-08-20 18:00:00',
        id: 11,
        likeCount: 1,
        location: '매산로1가',
        nickname: '당당근',
        replyCount: 0,
      },
    ],
  },
  {
    content:
      '왜 이렇게 가여운 냥이와 댕이들이 많은지 마음만 착잡해지네요. 모두 행복해졌으면 좋겠어요.',
    createdAt: '2026-08-20 18:00:00',
    id: 2,
    likeCount: 1,
    location: '권선2동',
    nickname: '언제나크리스마스',
    profileImageUrl: 'https://picsum.photos/seed/community-comment-2/72/72',
    replyCount: 0,
  },
  {
    content: '제 말이요. 저기 높아서 너무 위험해 보여요.',
    createdAt: '2026-08-20 18:00:00',
    id: 3,
    likeCount: 1,
    location: '매산로1가',
    nickname: '당당근',
    replyCount: 0,
  },
  {
    content: '구조 방법이 없을까요?',
    createdAt: '2026-08-20 20:00:00',
    id: 4,
    likeCount: 1,
    location: '오목천동',
    nickname: '포로리아옹',
    replyCount: 0,
  },
];

export const POPULAR_COMMUNITY_ITEMS: readonly CommunityItem[] = [
  {
    authorProfile: {
      location: '인천광역시 미추홀구 용현1,4동',
      nickname: '동물친구',
      warmth: 41.7,
    },
    id: 101,
    title: '동물',
    content: '동네에서 만난 귀여운 동물 이야기를 나눠요.',
    category: '취미',
    createdAt: '2026-08-14 15:00:00',
    location: '미추홀구 용현1,4동',
    tags: '동물,취미,이웃이야기',
    likeCount: 6,
    commentCount: 5,
    viewCount: 0,
    imageUrl: 'https://picsum.photos/seed/neighborhood-animal/240/240',
  },
  {
    authorProfile: {
      location: '경기도 고양시 일산동구 고봉동',
      nickname: '봉구네',
      warmth: 48.9,
    },
    id: 102,
    title: '**강아지 봉구 찾습니다**',
    content: '산책 중 사라진 봉구를 보신 분은 제보 부탁드려요.',
    category: '분실/실종',
    createdAt: '2026-08-16 15:00:00',
    location: '고양시 일산동구 고봉동',
    tags: '강아지,실종동물,봉구',
    likeCount: 6,
    commentCount: 5,
    viewCount: 0,
    imageUrl: 'https://picsum.photos/seed/neighborhood-dog/240/240',
  },
  {
    authorProfile: {
      location: '부산광역시 수영구 광안제1동',
      nickname: '광안리한달살이',
      warmth: 36.8,
    },
    id: 103,
    title: '광안리에 한달 살기 왔어요~',
    content: '광안리에서 지내며 발견한 동네 풍경을 공유합니다.',
    category: '동네풍경',
    createdAt: '2026-08-15 15:00:00',
    location: '수영구 광안제1동',
    tags: '광안리,한달살기,동네풍경',
    likeCount: 6,
    commentCount: 5,
    viewCount: 0,
    imageUrl: 'https://picsum.photos/seed/neighborhood-beach/240/240',
  },
  {
    authorProfile: {
      location: '충청북도 청주시 흥덕구 봉명1동',
      nickname: '궁금한이웃',
      warmth: 35.9,
    },
    id: 104,
    title: '이게 뭐에쓰는 녀석일까요?',
    content: '집에서 발견했는데 어디에 사용하는 물건인지 궁금해요.',
    category: '일반',
    createdAt: '2026-08-14 14:00:00',
    location: '청주시 흥덕구 봉명1동',
    tags: '궁금해요,정보공유,동네질문',
    likeCount: 6,
    commentCount: 5,
    viewCount: 0,
    imageUrl: 'https://picsum.photos/seed/neighborhood-question/240/240',
  },
  {
    authorProfile: {
      location: '경상북도 포항시 남구 지곡동',
      nickname: '포항새내기',
      warmth: 37.1,
    },
    id: 105,
    title: '안녕하세요 포항에 온지 얼마 안된 학생입니다',
    content: '포항에서 자주 가는 맛집이나 산책 코스를 추천해 주세요.',
    category: '맛집',
    createdAt: '2026-08-14 13:00:00',
    location: '포항시 남구 지곡동',
    tags: '포항,맛집추천,동네정보',
    likeCount: 6,
    commentCount: 5,
    viewCount: 0,
    imageUrl: 'https://picsum.photos/seed/neighborhood-student/240/240',
  },
  {
    authorProfile: {
      location: '경기도 안산시 상록구 이동',
      nickname: '하늘수집가',
      warmth: 43.5,
    },
    id: 106,
    title: '하늘이.. 반으로',
    content: '오늘 동네에서 본 신기한 하늘 풍경이에요.',
    category: '동네풍경',
    createdAt: '2026-08-14 12:00:00',
    location: '안산시 상록구 이동',
    tags: '하늘,풍경,오늘의동네',
    likeCount: 6,
    commentCount: 5,
    viewCount: 0,
    imageUrl: 'https://picsum.photos/seed/neighborhood-sky/240/240',
  },
  {
    authorProfile: {
      location: '인천광역시 미추홀구 도화동',
      nickname: '도화동고민상담소',
      warmth: 39.8,
    },
    id: 107,
    title: '이게 뭘까요? ㅠㅠㅠㅠ',
    content: '처음 보는 물건이라 아시는 이웃분께 여쭤봅니다.',
    category: '고민/사연',
    createdAt: '2026-08-13 15:00:00',
    location: '미추홀구 도화동',
    tags: '고민,궁금해요,이웃도움',
    likeCount: 6,
    commentCount: 5,
    viewCount: 0,
    imageUrl: 'https://picsum.photos/seed/neighborhood-mystery/240/240',
  },
  {
    authorProfile: {
      location: '경기도 용인시 처인구 역북동',
      nickname: '역북동집사',
      warmth: 46.2,
    },
    id: 108,
    title: '이 친구 아시는 분 계신가요..',
    content: '동네에서 만난 친구인데 주인을 찾고 있는 것 같아요.',
    category: '반려동물',
    createdAt: '2026-08-13 14:00:00',
    location: '용인시 처인구 역북동',
    tags: '반려동물,제보,이웃찾기',
    likeCount: 6,
    commentCount: 5,
    viewCount: 0,
    imageUrl: 'https://picsum.photos/seed/neighborhood-friend/240/240',
  },
  {
    authorProfile: {
      location: '서울특별시 영등포구 당산제1동',
      nickname: '당산산책러',
      warmth: 40.6,
    },
    id: 109,
    title: '길 가다가 애견미용실 있길래 봤....',
    content: '산책하다 발견한 귀여운 동네 풍경을 공유해요.',
    category: '동네풍경',
    createdAt: '2026-08-13 13:00:00',
    location: '영등포구 당산제1동',
    tags: '애견미용,강아지,동네풍경',
    likeCount: 6,
    commentCount: 5,
    viewCount: 0,
    imageUrl: 'https://picsum.photos/seed/neighborhood-shop/240/240',
  },
  {
    authorProfile: {
      location: '대구광역시 중구 남산1동',
      nickname: '남산동박사',
      warmth: 38.7,
    },
    id: 110,
    title: '무슨 벌레인가요?...',
    content: '집 근처에서 발견했는데 어떤 벌레인지 궁금합니다.',
    category: '생활/편의',
    createdAt: '2026-08-16 14:00:00',
    location: '중구 남산1동',
    tags: '벌레,생활정보,궁금해요',
    likeCount: 6,
    commentCount: 5,
    viewCount: 0,
    imageUrl: 'https://picsum.photos/seed/neighborhood-bug/240/240',
  },
];
